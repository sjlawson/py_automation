...
class Proxy(threading.Thread):
    ...
    def _process_request(self, data):
        ...
        if self.server and not self.server.closed:
            ...
            return
        # logger.info("DATA 42: %s" % str(data))
        # parse http request
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            # Hook redirect for mountebank imposter
            if self.request.url.path[:len('api.segment.io')] == b'api.segment.io':
                host = self.retarget_host
                port = self.retarget_port
...
class HTTP(TCP):
    ...
    def handle(self, client):
        proc = Proxy(client)
        proc.retarget_host = self.retarget_host
        proc.retarget_port = self.retarget_port
        proc.daemon = True
        proc.start()
